...
*/
   ...
   if (GenInterProfile) {//do not run docking: generate protein-ligand profile for statistic only
          //old:  fprintf(dlogfp,"%11.3f %12.3f\n",TChargeEnergy, Vtotal);
        //Modification
        //fprintf(dlogfp,"\t%.4f\t%.4f\t%.4f",eVdr,eHbond,eElec);
        fprintf(dlogfp,"\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n",
                eVdrTotal,eHbondTotal,eElecTotal,InRange/m1->count,Vtotal);
        oss_profile << endl;
        //oss_fitness << tab << eVdrTotal << tab << eHbondTotal << tab << eElecTotal << tab << InRange/m1->count << tab << Vtotal << endl;
...
double ComputeVDW95(Atoms *atom1, Atoms *atom2, double r)
{
    ...
    // if (Hbond == 1 && Ener < 0)
    //    Ener=5.0*Ener;
 // printf("\n r=%f E=%f H=%d",r,Ener,Hbond); getch();
    return(Ener);
}
...
double RMSD(Molecule *m)
{
    ...
    for (i=0;i<DrugAtomCount;i++) {
       ...
       // printf("\nDrugATOMS      %3d %8.3f %8.3f %8.3f",i,DrugATOMS[i].pos.x,DrugATOMS[i].pos.y,DrugATOMS[i].pos.z);
       // printf("\nDummyDrugATOMS %3d %8.3f %8.3f %8.3f",i,m->atom[i].pos.x,m->atom[i].pos.y,m->atom[i].pos.z);
    }
